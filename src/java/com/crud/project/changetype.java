...
public class changetype extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet changetype</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet changetype at " + request.getContextPath() + "</h1>");
            int acc=Integer.parseInt(request.getParameter("type"));
            int u_id=Integer.parseInt(request.getParameter("id"));
            try{
                UserDao udao=new UserDao(ConnectionDao.getCon());
                if(udao.editAcc(u_id,acc)){
